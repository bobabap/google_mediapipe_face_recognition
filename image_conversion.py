...
def rotate_img(image):
    ...
    for facial_landmarks in result.multi_face_landmarks:
        ...
        image  = cv2.warpAffine(image, rot_mat, image.shape[1::-1], flags=cv2.INTER_LANCZOS4, borderValue=(0,0,0))
...
def centerd_img(image, result):
    ...
    for facial_landmarks in result.multi_face_landmarks:
        ...
        image = cv2.warpAffine(image, M, (0, 0), flags=cv2.INTER_LANCZOS4, borderValue=(0,0,0))
...
def guide_image_landmark(p_height, p_width): # 변형할 이미지의 shape
    # _, g_image = url_img('guide_face.txt')
    # g_image = g_image[0]
    
    image = cv2.imread('guide_face.png', cv2.IMREAD_COLOR)
    g_image = image.copy()
    ...
    g_image = g_image[g_y_min-30:g_y_max+30, g_x_min-30:g_x_max+30]
    # -----------------------------------------------------------------
    
    g_image = cv2.resize(g_image, (p_width, p_height), interpolation=cv2.INTER_LANCZOS4)
    '''최종 가이드 사진 랜드마크 갱신'''
    ...
    return left_eye_x, left_eye_y, right_eye_x, right_eye_y, mid_nose_x, mid_nose_y, mid_chin_x, mid_chin_y, left_cheek_x, left_cheek_y, right_cheek_x, right_cheek_y, mid_head_x, mid_head_y
...
def geometric_deformation(image, result):
    ...
    image = image[y_min-30:y_max+30, x_min-30:x_max+30] # 얼굴을 자른 상태에서 변형하기위해 단 얼굴 자르면 다음 얼굴인식이 잘안됌 그래서 최종단계에서 잘라야함
    ...
    '''이미지 기하학 변형 부분'''
    # 가이드 얼굴 랜드마크 정면
    g_left_eye_x, g_left_eye_y, g_right_eye_x, g_right_eye_y, g_mid_nose_x, g_mid_nose_y, g_mid_chin_x, g_mid_chin_y, g_left_cheek_x, g_left_cheek_y, g_right_cheek_x, g_right_cheek_y, g_mid_head_x, g_mid_head_y = guide_image_landmark(height, width)
    ...
    image = cv2.warpAffine(image, M, (width - (g_mid_nose_x - mid_nose_x), height - (g_mid_nose_y - mid_nose_y)), flags=cv2.INTER_LANCZOS4, borderValue=(0,0,0))
    ...
    pts2 = np.float32([[g_left_eye_x, left_eye_y], [g_right_eye_x, right_eye_y], [g_mid_nose_x, mid_nose_y]]) # 변형 될 위치 삼각 좌표
    
    #---3 짝지은 3개의 좌표로 변환 행렬 계산
    mtrx = cv2.getAffineTransform(pts1, pts2)  #####################################
    
    #---4 어핀 변환 적용  민트색  
    image = cv2.warpAffine(image, mtrx, (int(width*1.5), height), flags=cv2.INTER_LANCZOS4, borderValue=(0,0,0)) #########################################
    # int(width - (g_mid_nose_x-mid_nose_x)), height - (g_mid_nose_y-mid_nose_y)
    ...
    geometrically_modified = cv2.cvtColor(cv2.cvtColor(geometrically_modified, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2GRAY)
    cv2.imshow('Image', geometrically_modified)
    cv2.waitKey(0)
    # ----------------------------------------------------
    return geometrically_modified


'''media.py에서 원본 profile image 하나씩 넣고 함수 순서대로 실행하는 부분'''
def conversion(file, image):
    # print(image.shape) # 원본 shape
    if image.shape[0] > 1500 or image.shape[1] > 1500: # 너비 높이 1500 이상이면 보간법으로 이미지 축소 
        image = cv2.resize(image, (0, 0), fx=0.4, fy=0.4, interpolation=cv2.INTER_LANCZOS4)
    ...
    final_image = geometric_deformation(centerd_image, Reset_result)
